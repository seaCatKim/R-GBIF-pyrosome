...
SOI values are reported monthly to match the temporal resolution of the chl-a and the outliers were treated the same as the chl-a data. Monthly SOI was interpolated to a daily resolution subset to the same period as the chl-a data. Monthly (30 days) and annual (360 days) moving average windows were tested for the SOI (@suppfig-soi-ma).
