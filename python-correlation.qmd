...
Title: Pyrosomes, *Pyrosoma atlanticum*: an important food source for coral reefs in Timor-Leste
...
## Correlations between Chlorophyll-a and the Southern Oscillation Index
...
The goal of the analysis was to determine whether there is any relationship between chl-a and the Southern Oscillation Index - a measure of El Nino Southern Oscillation. The chl-a data had missing values with variable gaps (@suppfig-chl-explore a) and outliers 2 standard deviations from the mean were removed and replaced with the median. In order to estimate the temporal correlation, we require our dataset to not have missing values. For this reason, we fill up the gaps with interpolated values. The comparison between the original and interpolated data shows the distribution and statistical properties of the interpolated data remain unchanged. Both cases have a skewed Gaussian (@suppfig-chl-explore b).

Using the interpolated data, we can analyse fast (monthly) and slow (yearly) changes throughout the time series by calculating the moving average for 30 days (monthly) and 360 days (yearly).\
...
ax1.legend();
ax1.set_title('a)', fontsize='small', loc='left')
...
# Add labels and title
ax2.set_title('b)', fontsize='small', loc='left')
ax2.set_xlabel('Data');
...
#ax3.grid(True)
ax3.set_title('c)', fontsize='small', loc='left')
...
SOI values are reported monthly so to match the temporal resolution of the chl-a the outliers 2 standard deviations from the mean were removed and replaced with the median

Monthly SOI was interpolated to a daily resolution for the same period as the chl-a data. Monthly (30 days) and annual (360 days) moving average windows were tested for the SOI (@suppfig-soi-ma).
...
plt.plot(interpolated_SOI, linestyle='-',color='red', label = 'Interpolated SOI', alpha = 0.5);
plt.plot(SOIma30, label='SOI moving average 30 days',linestyle='-');
plt.plot(SOIma360, label='SOI moving average 360 days',linestyle='-',color='green');
plt.xlim(0,10000);
plt.ylim(-4,4);
plt.xlabel('Months')
plt.ylabel('SOI')
...
The interpolated Souther Oscillation Index (SOI), 30 day moving average, and 360 day moving average for the historical time series starting Jan 1951.
:::
...
plt.grid(True)
plt.show()
#x = SOIma30[1:len(SOIma30)]
```

```{python}
#a_data = np.random.randn(1000)  # replace with actual dataset
#b_data = np.random.randn(1000)  # replace with actual dataset
a_data = SOIma360[1:len(SOIma360)]  # replace with actual dataset
b_data = ma360[1:len(ma360)]  # replace with actual dataset
...
Correlations between the chl-a and SOI data were calculated testing a lag of 1 to 360 days. SOI and chl-a are negatively correlated with a minimum correlation of -0.68 at a lag of 186 days (\~6 months) (@suppfig-chl-soi-correlation).

and obtain a correlation of -0.4. However, when we plot them, it is evident that there is a lag between the two indices. We calculate the correlation between the two indices as a function of the lag between them, finding the minimum happening with a lag of 186 days, as shown in Fig. XXX. We then plot the two normalized indices with a relative lag of 186 days between them and we can see that
...
p1.set_title('a)', fontsize='small', loc='left')
p1.grid(True)
...
p2.set_title('b)', fontsize='small', loc='left')
